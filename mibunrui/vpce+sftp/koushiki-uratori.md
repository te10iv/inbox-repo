...
[質問-1] PrivateLink用リソースの必要個数について
自社中継アカウントの
AZ-1に、IPv4用のNetworkLoadBalancerを1台だけ構築すれば良いか？
この1台に、
ターゲットグループをすべて設定すればよいか？

privatelinkはいくつ必要？
nlbひとつでOK?
vpceサービスはいくつ必要？
vpcエンドポイント（＝Interface型vpcエンドポイント）はいくつ必要？


[質問-2]
NetworkLoadBalancer一つ作れば、
IPv4もIPv6も、TCPもUDPもICMPも通信できる？

[質問-3]
上記通信はすべてPrivateLinkで行うべきか？
リソースゲートウェイを使った方が安いケースはあるか？

[質問-4]
そもそも、PrivateLinkを使わず、別のサービスを使った方がよいという可能性があるか？
(安い・可用性が高いなどの理由で)


[質問-5] クロスリージョン間通信の回避について　※構成図で説明してほしい
PrivateLinkでマルチAZ同士で接続するとしたら、
クロスリージョン間通信を発生させてしまうと、余計な費用がかかる？
であれば、クロスリージョン間通信を避けるには、どのように設計すればよいか？

逆に、どう設計構築してしまうと、
クロスリージョンで余分な費用が発生してしまうことになるか？

この場合のクロスリージョン間通信というのは
自社側だけの話か、他社も絡むことがありうるのか？
...
[質問-7]　 InterfaceエンドポイントのプライベートIPをDNSで伝達する方法は、どうするのがベストか？
